...
3. `cg tools generate` produces the complete GitHub Actions pipeline: container build/scan/sign + Cloudflare staging deploy + test gate + Cloudflare production deploy.
...
Use `cg images enrol` to add your image to `images.yaml`. The `--deploy` flags tell `tools generate` what CD pipeline to produce.
...
The `workflows` list declares the CD stages. `tools generate` uses this to wire up a staging deploy → test gate → production deploy sequence in the generated GitHub Actions pipeline.
...
Because your `images.yaml` entry declares a `deploy` section with Cloudflare Workers, `tools generate` emits a complete pipeline including the CD stages:
...
> **Do not edit these files by hand.** They carry an `# Auto-generated by CascadeGuard` header. Re-run `cg tools generate` after any `images.yaml` change.
...
Then re-run `cg images generate` to update state and `cg tools generate` to regenerate pipelines. The `autoRebuild: true` flag in `images.yaml` handles triggering rebuilds automatically on any future base image update.
...
```bash
cg tools pin --repo your-org/your-repo
```
...
- [CLI Reference](../reference/cli.md) — full reference for `cg images enrol`, `images check`, `tools generate`, and all other commands
